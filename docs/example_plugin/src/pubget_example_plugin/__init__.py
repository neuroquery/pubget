"""Example pubget plugin: plots the number of articles per publication year."""
import argparse
import logging
...
def plot_publication_dates(extracted_data_dir: Path) -> Tuple[Path, ExitCode]:
    ...
    extracted_data_dir
        ...
        created by `pubget.extract_data_to_csv`: it contains a file named
        `metadata.csv`.
    ...
    exit_code
        Status of this step, used by pubget command-line interface. In this
        simple example it is always COMPLETED. If a plugin doesn't want to
        import `pubget.ExitCode`, it can return an `int` instead -- see the
        values of the `pubget.ExitCode` enumaration for their meaning.
    """
...
class PlotPubDatesStep:
    """Plot publication dates as part of a pipeline (`pubget run`)."""
    ...
    def edit_argument_parser(self, argument_parser: ArgparseActions) -> None:
        ...
        When `pubget run` is invoked, this optional step is executed only if the
...
class PlotPubDatesCommand:
    """Plot publication dates as a standalone step (`pubget plot_pub_dates`)."""
    ...
    def run(
        ...
        """Execute the `pubget plot_pub_dates` command."""
...
def get_pubget_actions() -> Dict[str, List]:
    """Entry point used by pubget.
    ...
    for steps that must be added to the full pipeline (executed when `pubget run`
    is invoked), and "standalone_steps" for steps that run on their own (are
    added as separate subcommands, in this case `pubget plot_pub_dates`).

    The values are lists of objects that provide the same interface as
    `pubget.PipelineStep` and `pubget.Command` respectively: they have `name` and
